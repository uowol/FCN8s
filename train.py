...
def init_data(cfg) -> set:
    ...
    json_dir = os.path.join(cfg.data_dir, 'outputs_json')
    ...
    jsons = {
        os.path.relpath(os.path.join(root, filename), start=json_dir)
        for root, _dirs, filenames in os.walk(json_dir)
        for filename in filenames
        if os.path.splitext(filename)[1].lower() == '.json'
    }
    
    # check integrity
    pngs_prefix = {os.path.splitext(fname)[0] for fname in pngs}
    jsons_prefix = {os.path.splitext(fname)[0] for fname in jsons}
    
    assert len(jsons_prefix - pngs_prefix) == 0
    assert len(pngs_prefix - jsons_prefix) == 0
...
class XrayDataset(Dataset):
    def __init__(self, 
        ...
        dummy_ys = np.zeros(len(_filenames))
        gkf = GroupKFold(n_splits=5)

        filenames = []
        labelnames = []
        for i, (major_ind, minor_ind) in enumerate(gkf.split(_filenames, dummy_ys, groups)):
            if is_train:
                # let i-th fold validation set
                if i == 0: continue 
                filenames += list(_filenames[minor_ind])
                labelnames += list(_labelnames[minor_ind])
            else:
                filenames = _filenames[minor_ind]
                labelnames = _labelnames[minor_ind]
                break
    ...
    def __getitem__(self, index) -> (torch.Tensor, torch.Tensor):
        ...
        json_dir = os.path.join(self.cfg.data_dir, 'outputs_json')
        ...
        for anno in annos:
            ...
            # label per class, (H, W), numpy
            cls_label = np.zeros(image.shape[:2], dtype=np.uint8)
            cv2.fillPoly(cls_label, [points], 1)
            label[..., cls_ind] = cls_label
